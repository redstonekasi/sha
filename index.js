...
async function updateRepository(env) {
  try {
    await $`git clone ${config.repository} ${env}`;
    console.log(`git/${env}: repository cloned`);
  } catch {
    await $`git -C ${env} fetch`;
    console.log(`git/${env}: repository fetched`);
  }

  let rev;
  if (env === "staging") {
    rev = await $`git -C ${env} rev-parse main`;
    await $`git -C ${env} checkout ${rev}`;
  } else {
    rev = await $`git -C ${env} rev-list -1 main -- CHANGELOG.md`;
    await $`git -C ${env} checkout ${rev}`;
  }
  console.log(`git/${env}: checked out ${rev.stdout}`);

  await $({ cwd: env })`pnpm install --frozen`;
  console.log(`git/${env}: node modules updated`);
}
const updateAllRepositories = () => Promise.all(ENVS.map(updateRepository));

const state = {};
for (const env of ENVS) state[env] = {
  previous: null,
  current: null,
};
...
const stopWorker = (worker, env) => new Promise((res) => {
